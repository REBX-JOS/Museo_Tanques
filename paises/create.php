...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $check = $conn->prepare("SELECT 1 FROM paises WHERE id_pais = ?");
    $check->bind_param("i", $id_pais);
    $check->execute();
    $check->store_result();
    if ($check->num_rows > 0) {
        $msg = "Ya existe un país con ese ID.";
    } else {
        $sql = "INSERT INTO paises (id_pais, nombre_pais, capital, region) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("isss", $id_pais, $nombre_pais, $capital, $region);
        if ($stmt->execute()) {
...
<h2>Crear País</h2>
<form method="post">
    ID País: <input type="number" name="id_pais" required><br>
